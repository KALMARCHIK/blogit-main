...
def new_quiz(request):
    if request.GET.get('kolvo'):
        a=[QuestionForm()]*int(request.GET.get('kolvo'))
    ...
    if request.method=='POST':
        ...
        form1=Quiz.objects.create(name=names[0])
        variant=request.POST.getlist('variant')
        right=request.POST.getlist('right')
        for i in range(len(names[1:])):

            qst=Question.objects.create(name=names[i+1],qtype='single',quiz=form1,explanation='')
            for x in range(3):
                if int(right[0])==x:
                    Answer.objects.create(name=variant[x],question=qst,is_correct=True)
                else:
                    
                    Answer.objects.create(name=variant[x],question=qst)
            variant=variant[3:]
            right=right[1:]

        return redirect('quizzes:quizzes')
    context={'form1':form1,'a':a,}
    return render(request,'quizzes/new_quizzes.html',context)
